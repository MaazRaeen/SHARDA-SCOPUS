import {
  ...
  zip,
  zoom_default
} from "./chunk-LB4HAIFY.js";
import "./chunk-2SLMQRIO.js";
...
function styleMaybeRemove(id2, name) {
  ...
  return function() {
    var schedule = set(this, id2), on = schedule.on, listener = schedule.value[key] == null ? remove || (remove = styleRemove(name)) : void 0;
  ...
  *neighbors(i) {
  ...
  _clipSegment(x06, y06, x12, y12, c0, c1) {
    ...
    while (true) {
      ...
      let x4, y4, c4 = c0 || c1;
      if (c4 & 8) x4 = x06 + (x12 - x06) * (this.ymax - y06) / (y12 - y06), y4 = this.ymax;
      else if (c4 & 4) x4 = x06 + (x12 - x06) * (this.ymin - y06) / (y12 - y06), y4 = this.ymin;
      else if (c4 & 2) y4 = y06 + (y12 - y06) * (this.xmax - x06) / (x12 - x06), x4 = this.xmax;
  ...
  _project(x06, y06, vx, vy) {
    ...
    if (vy < 0) {
      ...
      if ((c4 = (this.ymin - y06) / vy) < t) y4 = this.ymin, x4 = x06 + (t = c4) * vx;
    } else if (vy > 0) {
      if (y06 >= this.ymax) return null;
      if ((c4 = (this.ymax - y06) / vy) < t) y4 = this.ymax, x4 = x06 + (t = c4) * vx;
    }
    if (vx > 0) {
      if (x06 >= this.xmax) return null;
      if ((c4 = (this.xmax - x06) / vx) < t) x4 = this.xmax, y4 = y06 + (t = c4) * vy;
    } else if (vx < 0) {
      if (x06 <= this.xmin) return null;
      if ((c4 = (this.xmin - x06) / vx) < t) x4 = this.xmin, y4 = y06 + (t = c4) * vy;
    }
    return [x4, y4];
  }
  _edgecode(x4, y4) {
...
var Delaunay = class _Delaunay {
  ...
  *neighbors(i) {
  ...
  find(x4, y4, i = 0) {
    ...
    const i0 = i;
    let c4;
    while ((c4 = this._step(i, x4, y4)) >= 0 && c4 !== i && c4 !== i0) i = c4;
    return c4;
  }
  _step(i, x4, y4) {
    const {
...
function circleStream(stream, radius, delta, direction, t0, t1) {
  ...
  if (t0 == null) {
    ...
    if (direction > 0 ? t0 < t1 : t0 > t1) t0 += direction * tau5;
  }
...
function circle_default() {
  var center2 = constant_default6([0, 0]), radius = constant_default6(90), precision = constant_default6(2), ring, rotate, stream = {
    point: point6
  };
...
function interpolate_default2(a4, b) {
  ...
  var interpolate = d ? function(t) {
    ...
    return [atan2(y4, x4) * degrees, atan2(z, sqrt2(x4 * x4 + y4 * y4)) * degrees];
  } : function() {
...
function resample(project, delta2) {
  function resampleLineTo(x06, y06, lambda04, a0, b02, c0, x12, y12, lambda12, a1, b12, c1, depth, stream) {
    ...
    if (d2 > 4 * delta2 && depth--) {
      var a4 = a0 + a1, b = b02 + b12, c4 = c0 + c1, m3 = sqrt2(a4 * a4 + b * b + c4 * c4), phi2 = asin(c4 /= m3), lambda22 = abs3(abs3(c4) - 1) < epsilon6 || abs3(lambda04 - lambda12) < epsilon6 ? (lambda04 + lambda12) / 2 : atan2(b, a4), p = project(lambda22, phi2), x22 = p[0], y22 = p[1], dx2 = x22 - x06, dy2 = y22 - y06, dz = dy * dx2 - dx * dy2;
...
function conicConformalRaw(y06, y12) {
  ...
  function project(x4, y4) {
    if (f > 0) {
      if (y4 < -halfPi3 + epsilon6) y4 = -halfPi3 + epsilon6;
    } else {
      if (y4 > halfPi3 - epsilon6) y4 = halfPi3 - epsilon6;
    }
...
function packSiblingsRandom(circles, random) {
  ...
  a4 = [b._], c4 = b;
  while ((c4 = c4.next) !== b) a4.push(c4._);
  c4 = packEncloseRandom(a4, random);
  for (i = 0; i < n; ++i) a4 = circles[i], a4.x -= c4.x, a4.y -= c4.y;
  return c4.r;
}
...
function pie_default() {
  var value = identity_default4, sortValues = descending_default, sort2 = null, startAngle = constant_default8(0), endAngle = constant_default8(tau6), padAngle = constant_default8(0);
  function pie(data) {
...
function zoom_default2() {
  ...
  zoom.translateTo = function(selection, x4, y4, p, event) {
    zoom.transform(selection, function() {
      ...
      return constrain(identity2.translate(p02[0], p02[1]).scale(t.k).translate(typeof x4 === "function" ? -x4.apply(this, arguments) : -x4, typeof y4 === "function" ? -y4.apply(this, arguments) : -y4), e, translateExtent);
    }, p, event);
  };
